...
public class FindPlanetServiceImpl implements FindPlanetService {
  ...
  public List<PlanetDto> executeByName(String planetName, int page, int size) {
    ...
    List<Planet> planets =
        repository.findByNameIgnoreCase(planetName, pageable).stream().collect(Collectors.toList());

    if (CollectionUtils.isEmpty(planets)) {
      throw new PlanetNotFoundException();
    }

    return planets.stream().map(planetsApiSearch::modelPlanetDto).collect(Collectors.toList());
  }
}
